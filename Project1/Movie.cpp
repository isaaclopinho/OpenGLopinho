...
void Movie::ResetVars() {
	baseticks = 0;
	audio_queue = NULL;
	audio_queue_tail = NULL;
}

void SDLCALL Movie::audio_callback(void *userdata, Uint8 *stream, int len) {
...
void Movie::queue_audio(const THEORAPLAY_AudioPacket *audio) {
...
void Movie::playfile(const char *fname, SDL_Window *screen, SDL_Renderer* renderer) {
	...
	texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING, video->width,
		video->height);
	...
	while (!quit && THEORAPLAY_isDecoding(decoder)) {
		...
		if (video && (video->playms <= now)) {
			...
			for (int i = 0; i < h; i++, y += w, dst += pitch)
				memcpy(dst, y, w);

			for (int i = 0; i < h / 2; i++, u += w / 2, dst += pitch / 2) 
				memcpy(dst, u, w / 2);

			for (int i = 0; i < h / 2; i++, v += w / 2, dst += pitch / 2) 
				memcpy(dst, v, w / 2);
